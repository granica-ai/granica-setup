...
```bash
# Get the SSH key generated by Terraform
terraform output -raw ssh_private_key > admin-key.pem
chmod 600 admin-key.pem

# With public IP (if public_ip_enabled = true):
ssh -i admin-key.pem granica@$(terraform output -raw public_ip)

# Without public IP: use Azure Serial Console
# Azure Portal → Virtual Machines → granica-admin-server-{server_name} → Help → Serial Console
```
